...
`missing [varlist]` forms. PR #22 currently evaluates a separate library-only
...
## In-progress slice: eager local-Parquet runtime boundary

PR #22 evaluates a Rust-owned `tabdat-runtime` session for one bounded path:
...
The contract, pinned Python execution evidence, local checks, native unsafe/license
review, and hosted acceptance state are recorded in
`_workspace/use-eager-parquet/` and ADR 0007. Until that PR is accepted, this is a
partial evaluation rather than a supported product runtime; all broad Phase 4
session, relation, load, inspect, transform, and reporting items remain unchecked.
