...
#[test]
fn keep_requires_an_active_dataset_without_initializing_a_backend() {
...
#[test]
fn keep_projects_requested_columns_and_preserves_rows() {
  ...
  let result = session
    .execute(parse_command("keep sex `age`").expect("projection should parse"))
    .expect("projection should execute");
  let ExecutionResult::Keep(keep) = result else {
    panic!("keep should return a Keep result");
  };
  assert_eq!(keep.dataset.source, fixture.parquet);
  assert_eq!(keep.dataset.row_count, 3);
  assert_eq!(
    keep
      .dataset
      .columns
...
#[test]
fn keep_preserves_state_when_a_requested_column_is_unknown() {
  let fixture = Fixture::new();
  let mut session = Session::new();
  session
    .execute(fixture.command())
  ...
  assert_eq!(
    session
      ...
      .unwrap_err(),
    RuntimeError::KeepUnknownVariable {
      variables: vec!["missing".to_owned(), "absent".to_owned()],
    }
  );
...
#[test]
fn keep_supports_empty_relations_and_duplicate_projection_names() {
  let fixture = Fixture::new();
  ...
  let result = session
    .execute(parse_command("keep age age").expect("duplicate projection should parse"))
    .expect("duplicate projection should execute");
  let ExecutionResult::Keep(keep) = result else {
    panic!("keep should return a Keep result");
  };
  assert_eq!(keep.dataset.row_count, 0);
  assert_eq!(
    keep
      .dataset
      .columns
