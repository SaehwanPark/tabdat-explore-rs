...
a syntax-only parser for a few control, status, inspection, and diagnostic
commands (`help`, `status`, `exit`, `describe`, `doctor`, `count`, `head`, and
`tail`); no data engine or statistical backend is installed.
